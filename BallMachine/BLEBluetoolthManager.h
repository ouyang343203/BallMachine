...
@class CSIIHybridBridge;
@interface BLEBluetoolthManager : NSObject

@property (nonatomic, strong)CSIIHybridBridge *bridge;
...
////3.监听蓝牙适配器状态变化事件(available:蓝牙适配器是否可用,discovering:蓝牙适配器是否处于搜索状态)
//-(void)onBluetoothAdapterStateChange;
////4.向蓝牙低功耗设备特征值中写入二进制数据

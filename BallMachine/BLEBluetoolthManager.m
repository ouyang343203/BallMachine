...
-(void)createBLEConnection:(NSDictionary*)parameter callBack:(BluetoothBLEConnectCallback)ConnectCallback
{
    ...
    if ([name isEqualToString:peripheral.name]) {
        if ([identifier isEqualToString:peripheral.identifier.UUIDString]&&peripheral!=nil) {
            self.centerManager.scanForPeripherals().connectToPeripherals().begin();
            [self didConnectPeripheral];
        }else{
            ConnectCallback(@{@"code":@"10013",@"errMsg":@"连接 address 为空或者是格式不正确"});
        }
    }else{
        ConnectCallback(@{@"code":@"10004",@"errMsg":@"设备不存在"});
    }
...
-(void)writeBLECharacteristicValue:(NSDictionary*)parameter callBack:(BluetoothWriteBLECallback)writeCallBack
{
    ...
    if (peripheral == nil) {writeCallBack(@{@"code":@"10002",@"errMsg":@"没有找到指定设备"}); return;}
    NSArray<CBService*>*newServices = peripheral.services;
    if (newServices.count == 0){writeCallBack(@{@"code":@"10004",@"errMsg":@"没有找到指定服务"}); return;}
    ...
    for (CBService *service in newServices)
    {
        if ([service.UUID.UUIDString isEqualToString:serviceId])
        {
            ...
            if (newCharcteritic.count == 0) {writeCallBack(@{@"code":@"10005",@"errMsg":@"没有找到指定特征"}); return;}
...
-(void)getBLEDeviceServices:(NSDictionary*)parameter callBack:(BluetoothServicesCallback)serviceCallBack
{
    ...
    CBPeripheral * peripheral = (CBPeripheral*)[_deviceDic objectForKey:identifier];
    if (peripheral == nil){serviceCallBack(@{@"code":@"10002",@"errMsg":@"没有找到指定设备"}); return;}
      NSArray<CBService*>*newServices = peripheral.services;
    if (newServices.count == 0){serviceCallBack(@{@"code":@"10004",@"errMsg":@"没有找到服务"}); return;}
    for (CBService *service in newServices) {
    ...
    serviceCallBack(@{@"code":@"0",@"errMsg":@"",@"data":serverces});
}
...
-(void)getBLEDeviceCharacteristics:(NSDictionary*)parameter callBack:(BluetoothCharacteristicsCallback)characteristics
{
    ...
    CBPeripheral * peripheral = (CBPeripheral*)[_deviceDic objectForKey:identifier];
    if (peripheral == nil){characteristics(@{@"code":@"10002",@"errMsg":@"没有找到指定设备"}); return;}
    NSArray<CBService*>*newServices = peripheral.services;
    if (newServices.count == 0){characteristics(@{@"code":@"10004",@"errMsg":@"没有找到指定服务"}); return;}
    for (CBService *service in newServices) {
        if ([service.UUID.UUIDString isEqualToString:serviceId]) {
            NSArray <CBCharacteristic*> *newCharcteritic  = service.characteristics;
            if (newCharcteritic.count == 0) {characteristics(@{@"code":@"10005",@"errMsg":@"没有找到指定特征"}); return;}
            for (CBCharacteristic *charcteritic in service.characteristics) {
            ...
            characteristics(@{@"code":@"10004",@"errMsg":@"没有找到指定服务"}); return;
        }
    }
    characteristics(@{@"code":@"0",@"errMsg":@"",@"data":@{@"status":@"0",@"data":charcterArray}});
}
...
-(void)notifyBLECharacteristicValueChange:(NSDictionary*)parameter callBack:(BluetoothNotifyCharacteristicCallBlock)characteristicCallBack
{
    ...
    NSString *characteristicId = parameter[@"characteristicId"];
    CBPeripheral * peripheral = (CBPeripheral*)[_deviceDic objectForKey:identifier];
    if (peripheral == nil){characteristicCallBack(@{@"code":@"10002",@"errMsg":@"没有找到指定设备"}); return;}
    NSArray<CBService*>*newServices = peripheral.services;
    if (newServices.count == 0){characteristicCallBack(@{@"code":@"10004",@"errMsg":@"没有找到指定服务"}); return;}
    for (CBService *service in newServices) {
        if (![service.UUID.UUIDString isEqualToString:serviceId]) {
            characteristicCallBack(@{@"code":@"10004",@"errMsg":@"没有找到指定服务"}); return;
        }
        NSArray <CBCharacteristic*> *newCharcteritic  = service.characteristics;
        if (newCharcteritic.count == 0) {characteristicCallBack(@{@"code":@"10005",@"errMsg":@"没有找到指定特征"}); return;}
        for (CBCharacteristic *charcteritic in newCharcteritic) {
            if (![charcteritic.UUID.UUIDString isEqualToString:characteristicId]) {
                characteristicCallBack(@{@"code":@"10005",@"errMsg":@"没有找到指定特征"});
                return;
            }
            [self.centerManager notify:peripheral characteristic:charcteritic block:^(CBPeripheral *peripheral, CBCharacteristic *characteristics, NSError *error) {
                characteristicCallBack(@{@"code":@"0",@"errMsg":@""});
            }];
        }
    }
    ...
    [self.centerManager setBlockOnCentralManagerDidUpdateState:^(CBCentralManager *central) {
        switch (central.state) {
            case CBCentralManagerStateUnknown:
                weakSelf.stateCallBack(@{@"code":@"0004",@"errMsg":@"状态未知",@"data":@(false)});
              break;
            case CBCentralManagerStateResetting:
                weakSelf.stateCallBack(@{@"code":@"0005",@"errMsg":@"蓝牙断开即将重置",@"data":@(false)});
              break;
            case CBCentralManagerStateUnsupported:
                weakSelf.stateCallBack(@{@"code":@"0002",@"errMsg":@"蓝牙未启",@"data":@(false)});
              break;
            case CBCentralManagerStateUnauthorized:
                weakSelf.stateCallBack(@{@"code":@"0003",@"errMsg":@"蓝牙未被授权",@"data":@(false)});
              break;
            case CBCentralManagerStatePoweredOff:
            {
                weakSelf.stateCallBack(@{@"code":@"0001",@"errMsg":@"蓝牙未启",@"data":@(false)});
            }
              break;
            case CBCentralManagerStatePoweredOn:
            {
                weakSelf.stateCallBack(@{@"code":@"0",@"errMsg":@"",@"data":@(true)});
            }
              break;
            default:
              break;
...
//        weakSelf.iscontent = NO;
        NSDictionary *data = @{@"code":@"10006",@"errMsg":@"当前连接已断开"};
//        [bridBridge callHandler:@"registerConnectStatusListener" data:data responseCallback:^(id responseData) {
//
//            NSLog(@"设备断开连接");
//        }];
    ...
    [self.centerManager setBlockOnDiscoverServices:^(CBPeripheral *peripheral, NSError *error) {
        ...
        if (peripheral.services.count> 0) {
            ...
            weakSelf.contentCallBack(@{@"code":@"10004",@"errMsg":@"没有找到指定服务"});
        }
    ...
    [self.centerManager setBlockOnDiscoverCharacteristics:^(CBPeripheral *peripheral, CBService *service, NSError *error) {
        if (service.characteristics.count> 0) {
            ...
            weakSelf.contentCallBack(@{@"code":@"10005",@"errMsg":@"没有找到指定特征"});
        }
...
//            }];
    ...
    [self.centerManager setBlockOnDidUpdateNotificationStateForCharacteristic:^(CBCharacteristic *characteristic, NSError *error) {
        NSLog(@"征值变化的通知");
        NSDictionary *data = @{@"code":@"0",@"errMsg":@"",@"data":characteristic.value};
//        [bridBridge callHandler:@"characterWriteStatusListener" data:data responseCallback:^(id responseData) {
//
//            NSLog(@"写入状态成功");
//        }];
